...
void Car::tick(const std::vector<Box> &colliders) {
	...
	for (const Box &box : colliders) {
		if (collision(posX-CAR_WIDTH/2, posY-CAR_HEIGHT/2, CAR_WIDTH, CAR_HEIGHT, angle, box.posX - box.width/2, box.posY - box.height/2, box.width, box.height, box.angle)) {
